...
from AIP import orginal_transform
...
if args.model_to_eval == 'DVBPR':
    ...
    checkpoint = torch.load('./models/' + data_for_experiment + '_k100_DVBPR.tar', map_location=map_location)
    ...
    eval_root = './adv_output1/DVBPR/' + args.adv_item_path + '/'
...
if args.model_to_eval == 'VBPR':
    ...
    if data_for_experiment == 'amazon':
        ...
        dataset = np.load(data_root + dataset_name, encoding='bytes',allow_pickle=True)
    ...
    eval_root = './adv_output1/VBPR/' + args.adv_item_path + '/'
    def default_loader(path):
        img_pil =  Image.open((path)).convert('RGB')
...
if args.model_to_eval == 'AMR':
    ...
    eval_root = './adv_output1/AMR/' + args.adv_item_path + '/'
    def default_loader(path):
        img_pil =  Image.open((path)).convert('RGB')
        img_tensor = VBPR_test_transform(img_pil)
        return img_tensor
...
if args.model_to_eval == 'AlexRank':
    ...
    eval_root = './adv_output1/AlexRank/' + args.adv_item_path + '/'

...
parser.add_argument("-lambda1", "--lambda1", type=float, help="Weight of regulizer for user embeddings.",
                    default="0.001")
parser.add_argument("-learning_rate", "--lr", type=float, help="Training learning rate.", default="0.01")
parser.add_argument("-num_workers", "--numofworkers", type=int, help="Number of co-workers for dataloader.",
                    default="8")
...
if data_train == 'amazon':
    ...
    dataset = np.load('data/' + dataset_name, encoding='bytes', allow_pickle=True)
...
class testset(Dataset):
    def __init__(self):
        from gen_candidate_set import test_ls
        self.target = test_ls
